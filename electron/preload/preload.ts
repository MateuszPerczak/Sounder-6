...
contextBridge.exposeInMainWorld("api", {
  ...
  openFilePicker: (): Promise<Buffer | undefined> =>
    ipcRenderer.invoke("open-file-picker"),
  scanFolders: (folders: string[]): Promise<string[]> =>
    ipcRenderer.invoke("scan-folders", folders),
  getSongMetadata: (path: string): Promise<void> =>
    ipcRenderer.invoke("get-song-metadata", path),
  getSongBytes: (path: string): Promise<void> =>
    ipcRenderer.invoke("get-song-bytes", path),
});
